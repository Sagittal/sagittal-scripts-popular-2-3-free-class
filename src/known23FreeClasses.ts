...
const computeKnownPopular23FreeClasses = (): Popular23FreeClass[] => {
    const knownPopular23FreeClasses = readLines("src/input/knownPopular23FreeClasses.txt" as Filename).map(
        (knownPopular23FreeClassIo: Io): Two3FreeClass => {
            return parse23FreeClass(knownPopular23FreeClassIo)
        },
    )

    return knownPopular23FreeClasses.map((two3FreeClass: Two3FreeClass): Popular23FreeClass => {
        return computePopular23FreeClass(analyze23FreeClass(two3FreeClass))
    })
}

export { computeKnownPopular23FreeClasses }
